...
def to_mps(mpo):
    ...
    return [N.reshape(mt,[mt.shape[0],pdim*pdim,mt.shape[3]]) for mt in mpo]

def from_mps(mps):
    """
    squaren physical indices of MPS to MPO
    """
    pdim=int(math.sqrt(mps[0].shape[1]))
    assert pdim*pdim==mps[0].shape[1]
    
    return [N.reshape(mt,[mt.shape[0],pdim,pdim,mt.shape[2]]) for mt in mps]
